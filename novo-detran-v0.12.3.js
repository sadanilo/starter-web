...
(function () {
  ...
  if (
    ...
    let hideButton = htmlToElements(`
      <button id="hideFines" class="ui-button ui-widget ui-state-default ui-corner-all ui-button-text-icon-left btn btn-warning btn-sm hidden-print">
        <span class="ui-button-icon-left ui-icon ui-c fa fa-eye-slash"></span>
        <span class="ui-button-text ui-c">Esconder</span>
      </button>
    `);
    ...
    waitForKeyElements("#dtMultasWS_data", function (elem) {
      //Adiciona o botão de esconder as multas dos outros órgãos
      let divPlate = document.querySelector("#formMultas > div.row.hidden-print > div.col-lg-10.col-md-12.col-sm-12.col-xs-12");
      divPlate.classList.remove("col-lg-10");
      divPlate.classList.add("col-lg-8");

      let divButton = document.querySelector(
        "#formMultas > div.row.hidden-print > div.col-lg-2.col-md-12.col-sm-12.col-xs-12.text-right.text-center-xs.text-center-sm.text-center-md"
      );
      divButton.classList.remove("col-lg-2");
      divButton.classList.add("col-lg-4");
      divButton.append(hideButton);

      hideButton.onclick = function (e) {
        e.preventDefault();
        for (let row of elem.rows) {
          if (row.cells[6].innerText.trim() != "TERESINA") {
      ...
      //Adiciona uma coluna a mais para acomodar os botões de ação
      let actionsTableHead = `<th class="ui-state-default w-80 hidden-print" role="columnheader" aria-label="Local" scope="col"><span class="ui-column-title">Ações</span></th>`;
      let actionsTableFoot = `<td class="ui-state-default uix-datatable-date"></td>`;
      document.querySelector("#dtMultasWS_head > tr").append(htmlToElements(actionsTableHead));
      document.querySelector("#dtMultasWS_foot > tr").append(htmlToElements(actionsTableFoot));

      //Adiciona os botões de ações
      for (let row of elem.rows) {
        let auto = row.cells[4].querySelector("span").innerText;

        if (row.cells[6].innerText.trim() === "TERESINA") {
          row.style = `background-color: lightgoldenrodyellow;`;
          row.append(htmlToElements(`<td role="gridcell" style="text-align: center; width: 50px"></td>`));

          // // Check if the string starts with 'ER' or 'TR'
          // if (auto.startsWith("ER") || auto.startsWith("TR")) {
          //   // Skip the rest of the loop and continue to the next iteration
          //   continue;
          // }

          if (row.cells[8].innerText.trim() === "Registrada") {
            row.cells[12].innerHTML = `<a style='display:block; margin-top:5px' target='_blank' href='https://sistemas.detran.pi.gov.br/renainf-web/restrito/cadastro/notificacaoAutuacao/createNotificacaoAutuacao.jsf?auto=${auto}&placa=${placa}&renavam=${renavam}'><i class="fa fa-2x fa-pencil-square-o" aria-hidden="true"></i></a>
            <a style='display:block; margin-top:5px' target='_blank' href='https://sistemas.detran.pi.gov.br/renainf-web/restrito/cadastro/notificacaoPenalidade/createNotificacaoPenalidade.jsf?auto=${auto}&placa=${placa}&renavam=${renavam}'><i class="fa fa-2x fa-usd" aria-hidden="true"></i></a>`;
          }

          if (row.cells[8].innerText.trim() === "Autuada") {
            row.cells[12].innerHTML = `<a style='display:block; margin-top:5px' target='_blank' href='https://sistemas.detran.pi.gov.br/renainf-web/restrito/cadastro/notificacaoPenalidade/createNotificacaoPenalidade.jsf?auto=${auto}&placa=${placa}&renavam=${renavam}'><i class="fa fa-2x fa-usd" aria-hidden="true"></i></a>`;
          }
          let linkOcorrencia = htmlToElements(`
              <a style='display:block; margin-top:5px' target='_blank' title='Cadastrar Ocorrência' href='https://sistemas.detran.pi.gov.br/renainf-web/restrito/servico/ocorrencia/createOcorrencia.jsf?auto=${auto}&placa=${placa}&renavam=${renavam}'><i style="margin-left: 5px;" class="fa fa-2x fa-indent" aria-hidden="true"></i></a>
            `);
          row.cells[12].append(linkOcorrencia);
        }
      }
    });
  }
