(function () {
  ...
  fab.querySelector("#copyActionDate").onclick = async function (e) {
    ...
    let dateFinal = iframeArvore.contentWindow.document
      .querySelector("body > table > tbody > tr > td > p")
      .innerText.match(/\d{1,2}\/\d{1,2}\/\d{4}/);
    data.dateFinal = dateFinal[0];
    let typeOfProccess = "";
    typeOfProccess = iframeArvore.contentWindow.document
      .querySelector("body > p.Texto_Justificado_Recuo_Primeira_Linha")
      .innerText.match(/\s(INDEFERIR|DEFERIR|NÃO CONHECER)\s/)
      ? iframeArvore.contentWindow.document
          .querySelector("body > p.Texto_Justificado_Recuo_Primeira_Linha")
          .innerText.match(/\s(INDEFERIR|DEFERIR|NÃO CONHECER)\s/)[0]
          .trim()
      : false;
    data.typeOfProccess = typeOfProccess;
    let savedObj = tryParseJSONObject(await navigator.clipboard.readText());
    if (savedObj) {
      savedObj.dateFinal = dateFinal[0];
      savedObj.typeOfProccess = typeOfProccess;
      await navigator.clipboard.writeText(JSON.stringify(savedObj));
    } else {
      await navigator.clipboard.writeText(JSON.stringify(data));
      savedObj = data;
  ...
  fab.querySelector("#copyFinePlate").onclick = async function (e) {
    ...
    let iframeDocumento = document.querySelector("#ifrVisualizacao");
    let iframeArvore = iframeDocumento.contentWindow.document.querySelector("#ifrArvoreHtml");
    let fine = iframeArvore.contentWindow.document.querySelector("body > p:nth-child(5)").innerText;
    let plate = iframeArvore.contentWindow.document.querySelector("body > p:nth-child(6)").innerText;
